...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      try {
        ...
        if (docSnap.exists()) {
          currentSchedule = docSnap.data();
          renderForm(currentSchedule);
        } else {
          alert("予定が見つかりません");
          window.location.href = 'calendar.html';
        }
  ...
  document.getElementById('save-btn').addEventListener('click', async () => {
    ...
    if (document.getElementById('input-title')) updateData.title = document.getElementById('input-title').value;
    if (document.getElementById('input-partner')) updateData.partner = document.getElementById('input-partner').value;
    if (document.getElementById('input-time')) updateData.time = document.getElementById('input-time').value;
    if (document.getElementById('input-detail')) updateData.detail = document.getElementById('input-detail').value;
    
    if (document.querySelector('.cb-member')) {
      updateData.members = Array.from(document.querySelectorAll('.cb-member:checked')).map(cb => cb.value);
    }
    
    if (document.querySelector('input[name="customColor"]:checked')) {
      updateData.customColor = document.querySelector('input[name="customColor"]:checked').value;
    }

    try {
      const docRef = doc(db, "schedules", scheduleId);
      // updateDocを使うことで、元の作成者(author)などの情報は上書きされずに保持される
